...
export class ExportDocService {
   ...
   * @param author Document author
   */
  exportAsCSV(name: string, data: any) {
  ...
  exportTablePDF(name, companyName, productName, category, dateRange, columns: JsPDFAutoTableColumn[], body: any[], ) {
    const doc = new JSPDF();
    // Or use javascript directly:
    doc.setFontSize(12);
    // doc.text("Products", 15, 15);
    // doc.text("Company Name: " + companyName, 15, 22);
  ...
  genericExportTablePDF(reportTitle, fileName, companyName, columns: JsPDFAutoTableColumn[], body: any[], ) {
    const doc = new JSPDF();
    // Or use javascript directly:
